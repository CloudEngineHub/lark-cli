...
const DEFAULT_BASE_TOKEN = "B1mRblqjzalTgnswzilcyOA6nVe";
const DEFAULT_TABLE_ID = "tbl0XN0aM4RUppI1";
const DEFAULT_VIEW_ID = "vewJcOBB1a";
...
Options:
  ...
  --executor-profile <name>   lark-cli profile used for blind baseline search
...
judge, or optimizer phases itself.
...
function valueToList(value) {
  if (Array.isArray(value)) {
    return value.map((item) => valueToString(item).trim()).filter(Boolean);
  }
...
function convertDataset(baseData, subset) {
  ...
  for (let i = 0; i < baseData.data.length; i += 1) {
    ...
    if (adoption.length > 0 && !adoption.includes("采纳")) {
      continue;
    }
