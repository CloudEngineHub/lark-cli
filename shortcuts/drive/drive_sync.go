...
var DriveSync = common.Shortcut{
	...
	Execute: func(ctx context.Context, runtime *common.RuntimeContext) error {
		...
		if driveSyncNeedsCreateScope(plannedUploads, folderCache) {
...
func driveSyncAskConflict(relPath string, runtime *common.RuntimeContext) (string, error) {
	...
	case "r", "remote", "remote-wins":
		return driveSyncOnConflictRemoteWins, nil
	default:
		return "", fmt.Errorf("invalid conflict choice for %q: %q (expected one of remote/local/keep/skip)", relPath, strings.TrimSpace(line))
	}
}
...
func driveSyncNeedsCreateScope(uploadPaths []string, folderCache map[string]string) bool {
