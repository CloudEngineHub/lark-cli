...
`docs +create` / `docs +update` 并非支持全部 block 类型。`<synced_reference>` / `<synced_source>` / `<bitable>` / `<base_ref>` / `<okr>` 这五类**不可创建**，写入路径会静默跳过；但它们可以通过 fetch 读出、通过 `block_move_after` 在文档间搬动。生成内容前请先读：

- [`references/lark-doc-block-type-limits.md`](references/lark-doc-block-type-limits.md) — 不可创建 block 清单、各自的 XML 形态、`<!-- Unsupported block type -->` 占位符来源与推荐处理方式。
